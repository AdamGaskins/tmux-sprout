    ...
    const char** reason)
{
    ...
    for (int i = 0; i < nkey; i++) {
        if (tab->u.tab.len[i] == newkey.len && 0 == memcmp(tab->u.tab.key[i], newkey.ptr, newkey.len)) {
    ...
    toml_datum_t* value = REALLOC(tab->u.tab.value, sizeof(*value) * align8(nkey + 1));
    if (!pkey || !plen || !value) {
...
{
    return &p[4] < endp && isdigit(p[0]) && isdigit(p[1]) && isdigit(p[2]) && isdigit(p[3]) && p[4] == '-';
}
